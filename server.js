...
const envApiKey = process.env.DEEPSEEK_API_KEY || process.env.OPENAI_API_KEY || "";
const envBaseUrl = (process.env.DEEPSEEK_BASE_URL || process.env.OPENAI_BASE_URL || defaultBaseUrl).replace(/\/$/, "");
const envModel = process.env.DEEPSEEK_MODEL || process.env.OPENAI_MODEL || defaultModel;
...
async function deepseekAnalyze(text, userState, perspective, config = {}) {
  const apiKey = config.apiKey || envApiKey;
  const baseUrl = (config.baseUrl || envBaseUrl).replace(/\/$/, "");
...
async function handleAnalyze(req, res) {
  try {
    ...
    if (!String(text).trim()) return send(res, 400, JSON.stringify({ error: "text is required" }));

    const apiKey = String(requestApiKey || envApiKey).trim();
    const baseUrl = String(requestBaseUrl || envBaseUrl).trim();
    const model = String(requestModel || envModel).trim();

    if (!apiKey) {
      return send(res, 200, JSON.stringify({ provider: "local", analysis: localAnalyze(text) }));
    }

    try {
      const analysis = await deepseekAnalyze(text, userState, perspective, { apiKey, baseUrl, model });
      return send(res, 200, JSON.stringify({ provider: `DeepSeek ${model}`, analysis }));
    } catch (error) {
      return send(
        res,
        200,
...
function startServer(options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(listenPort, host, () => {
      ...
      if (!quiet) {
        console.log(`Evidence Dot demo: http://localhost:${actualPort}`);
        console.log(envApiKey ? `LLM mode: DeepSeek Chat Completions (${envModel})` : "LLM mode: local fallback, or enter DeepSeek API settings in the page");
      }
